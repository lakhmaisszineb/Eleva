...
Analyzes the Observation (including RFM + core metrics)
and produces DetectedIssue list (problems & opportunities).
"""
...
def detect_node(state: DecisionState) -> DecisionState:
    ...
    if at_risk and at_risk.size > 0:
        add_issue(
            type_=IssueType.PROBLEM,
            title="Customers at risk of churning (RFM)",
            description=(
    ...
    if lost and lost.size > 0:
        add_issue(
            type_=IssueType.PROBLEM,
            title="Lost or churned customers (RFM)",
            description=(
                f"{lost.size} client(s) ({lost.percentage}%) sont classés "
                "'Lost / Churned'. Une campagne de win-back peut être pertinente."
            ),
    ...
    if repeat is not None and float(repeat) < 0.30:
        add_issue(
            ...
            description=(
                ...
                "Des actions de fidélisation / 2e commande sont prioritaires."
            ),
            priority=Priority.MEDIUM,
    ...
    # 6. Low ROI campaigns
    low_roi_insights = [i for i in obs.raw_insights if "faible ROI" in i.lower() or "low roi" in i.lower()]
    if low_roi_insights:
        add_issue(
            type_=IssueType.PROBLEM,
    ...
    if new_seg and new_seg.size > 0:
        add_issue(
            type_=IssueType.OPPORTUNITY,
            title="Convert new / promising customers",
            description=(
                f"{new_seg.size} client(s) 'New / Promising'. "
                "Une campagne de bienvenue / 2e commande peut les faire progresser."
            ),
            priority=Priority.MEDIUM,
            related_kpis=[],
            evidence=[
